...
export default class PointerLockControls {
    ...
    if (this.controlsEnabled) {
      ...
      this.raycasters = {
        ...
        bottom: new THREE.Raycaster(
          ...
          new THREE.Vector3(0, -1, 0),
          0,
          this.velocity.y < 0 && -this.velocity * delta < 1
            ? -this.velocity.y * (delta + 0.1)
            : 1
        ),
      };
      ...
      for (const prop in this.raycasters) {
        const hasIntersects =
          this.raycasters[prop].intersectObjects(this.objects).length > 0;
        if (prop == "bottom") {
          this.raycasters[prop].ray.origin.y +=
            -1 + this.velocity.y < 0 && -this.velocity * delta < 1
              ? -this.velocity.y * (delta + 0.1)
              : 1;
        }
      ...
      if (!canMoveLeft && this.object.position.x < prevPos.x)
        this.object.position.x = prevPos.x;
      if (!canMoveRight && this.object.position.x > prevPos.x)
        this.object.position.x = prevPos.x;
      if (!canMoveBack && this.object.position.z > prevPos.z)
        this.object.position.z = prevPos.z;
      if (!canMoveFront && this.object.position.z < prevPos.z)
        this.object.position.z = prevPos.z;
      if (!canMoveTop && this.object.position.y > prevPos.z)
        this.object.position.y = prevPos.y;
  ...
  updateCollisionObjects() {
    ...
    const currPos = this.object.position
      .clone()
      .floor()
